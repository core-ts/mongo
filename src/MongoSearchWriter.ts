...
export class MongoSearchWriter<T, ID, S> extends MongoWriter<T, ID> {
  constructor(protected find: (s: S, limit?: number, offset?: number|string, fields?: string[]) => Promise<SearchResult<T>>, public db: Db, collectionName: string, attributes: Attributes|string, toBson?: (v: T) => T, fromBson?: (v: T) => T) {
