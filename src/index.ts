...
export async function findWithMap<T>(collection: Collection, query: FilterQuery<T>, idName?: string, m?: StringMap, sort?: string | Array<[string, number]> | SortOptionObject<T>, limit?: number, skip?: number, project?: any): Promise<T[]> {
...
export function find<T>(collection: Collection, query: FilterQuery<T>, sort?: string | Array<[string, number]> | SortOptionObject<T>, limit?: number, skip?: number, project?: any): Promise<T[]> {
  return new Promise<T[]>((resolve, reject) => {
...
export async function insertMany<T>(collection: Collection, objs: T[], idName?: string): Promise<number> {
  try {
    const value = await collection.insertMany(revertArray(objs));
    if (idName) {
