...
var SearchRepository = (function () {
  function SearchRepository(db, collectionName, metadata, buildQuery, toBson, sort, q, excluding, buildSort) {
    this.toBson = toBson;
    this.sort = sort;
    if (metadata) {
  ...
  SearchRepository.prototype.search = function (filter, limit, page, fields) {
    ...
    return search_1.buildSearchResult(this.collection, query, so, limit, offset, fields, this.id, this.map, this.toBson);
  };
