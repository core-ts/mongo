...
function getFields(collection, field, values, noSort) {
  var query = {};
...
function upsert(collection, object, id, toBson, fromBson) {
  ...
  if (obj['_id']) {
    ...
    return collection.insert(object).then(function (r) {
      var v = r['insertedId'];
      if (v && id && id.length > 0) {
        object[id] = v;
      }
...
function buildProject(fields, all, notIncludeId) {
...
function fromPoints(s, geo, latitude, longitude) {
  ...
  return s.map(function (o) { return fromPoint(o, geo, latitude, longitude); });
}
...
function toPoints(s, geo, latitude, longitude) {
  ...
  return s.map(function (o) { return toPoint(o, geo, latitude, longitude); });
}
