...
exports.find = find;
function insert(collection, obj0, id, handleDuplicate, toBson, fromBson) {
  var obj = revertOne(obj0, id);
  if (toBson) {
    obj = toBson(obj);
  }
  return collection
...
function deleteFields(collection, object, filter, id) {
  ...
  var ft = { $pull: filter };
  return collection.findOneAndUpdate({ _id: obj["_id"] }, ft).then(function (res) {
    if (!res) {
      return 0;
    }
    return res.ok;
